...
**Linewidth caveat (derivation item, carry from SPEC.md §7.T4):** step 1 uses cavity κc for consistency with the pivot analysis, but the detuning-broadened denominator may want the **combined** cavity+spin linewidth (κc + κs). This changes the constant, not the 1/√Q scaling — pin it in the real derivation before publishing the coefficient.
...
2. **Q-vs-margin scaling (novel, conditional on §7.7 derivation).** The counter-intuitive result that higher-Q builds carry *less* thermal frequency margin (∝ 1/√Q). The sharpest scientific claim — but only if the derivation survives (do not lead with the napkin).
...
- **Recommended (if time):** output 2's *rigorous* Q-vs-margin derivation; §7.4 Bayesian re-inference (→ output 3 strengthened); §7.5 GP active learning; §7.6 GP-posterior emulator interval; §7T/§7.T5 calibration against the Cowley-Semple ODMR dataset — data now identified and in hand at scraped-points level (SPEC.md §11 item 5), so this has moved from "if it exists" to "pending the metadata asks."
...
9. **Q-vs-margin derivation** (§7.7 / SPEC.md §7.T4) — required before output 2 is claimed; the linewidth choice in §7.3b (cavity vs combined) is part of this.
