...
- **Rig geometry (recorded 2026-07-04 — planning assumption, pending Angus ratification).** Oxborrow's description, explicitly his-understanding-unconfirmed: thin Pc:PTP flake stuck to a microscope slide for handling, interrogated through a microscope objective, spot possibly microns — i.e. **not a bulk cylinder**. Checked against the published record this pass (Mena et al., PRL 133, 120801 (2024), Methods as printed in arXiv:2402.07572; Mann et al., JACS 147, 22911 (2025) = arXiv:2503.24341):
...
5. **Cowley-Semple ODMR dataset — RESOLVED (identity + existence), metadata pending.** What exists (from the 2026-06-26 "linewidths" thread, Glasgow/MIT/Imperial):
   ...
   **Routing update (2026-07-04): asks re-route to Angus directly.** Oxborrow makes the introduction **Monday 2026-07-06 — he asked to be reminded that morning; send the reminder.** Question list for Angus, priority order (informed by the §7.T5 rig-geometry note; the published-record partial answers are in parentheses):
   ...
   2. **Substrate material and mounting/adhesive.** (Published: glass slide + paraffin wax — same for these samples? *Mann SI 2026-07-04: mounting unstated there; Mena remains the only published mounting.*)
   3. **Which rig, and objective NA or measured spot size at sample.** (*Two configurations now on record — Mann SI 2026-07-04:* Mena 2024 cw path = LP520-SF15A diode + f = 30 mm doublet AC254-030-AB; Mann 2025 = 532 nm Finesse + C061TMD-A f = 11 mm NA 0.24 aspheric for cw *and* pulsed. Spot size stated in **neither**; your 50–100 mA drive-current axis looks diode-like — confirm which rig and the spot. NAP sample used C240TMD-B, f = 8 mm — different spot, do not pool.) Conditional on ask 1: **if plate**, the observable is the product k·w and an independent w is what factorises k out of it — decisive, to ×3 multiplicative (§7.T5 sweep); **if film**, moot (no k-lever). w also sets the w/t regime.
   4. **Laser power at sample and duty cycle:** wavelength (rig-dependent — 520 nm diode per Mena vs 532 nm Finesse per Mann; fold into ask 3); threshold current; slope efficiency (W/A) or measured power at each current. (*Mann SI 2026-07-04: no absolute power in print for either rig; cw duty = cw confirmed.*) *Without this, absolute ΔT(P) is uncalibrated; the ν(I) slope and the concentration ratio still are.*
   5. **Any time-resolved traces** (shift vs time after laser on/off). Existence unknown — not in either published paper; decides whether §7.T5 check 3c is live.
   6. **Raw ν(I)** for both concentrations (not scraped points), + uncertainties; raw spectra.
   7. **Co-authorship / acknowledgement expectations** for use of the dataset.
   *Rider (2026-07-05, volumetric appendix): the plate branch acquires a second factorisation input, pump penetration depth l_abs — needed only to ×3.0 worst-case. No new ask: covered by the metadata already in asks 1/3/4/6 plus a literature pull on our side — prefer measured penetration data (Takeda 2002 / Breeze-era) over computing l_abs from nominal doping, because nominal vs actual pentacene incorporation in Bridgman growth differ substantially; the same nominal-vs-actual systematic touches 3b's concentration ratio.*
