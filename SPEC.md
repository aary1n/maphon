...
Same pass: Singh et al., Nat. Commun. 16, 10530 (2025) — §11 item 7's must-pull — obtained and graded into §6T. RT-valid X–Z slope **−101 kHz/K**; the paper's headline 247 kHz/K is the T_xy transition *at the 193 K phase transition* and must never be used as the RT coefficient.
...
- **Rig geometry (recorded 2026-07-04 — planning assumption, pending Angus ratification).** Oxborrow's description, explicitly his-understanding-unconfirmed: thin Pc:PTP flake stuck to a microscope slide for handling, interrogated through a microscope objective, spot possibly microns — i.e. **not a bulk cylinder**. Checked against the published record this pass (Mena et al., PRL 133, 120801 (2024), Methods as printed in arXiv:2402.07572; Mann et al., JACS 147, 22911 (2025) = arXiv:2503.24341):
  ...
  - **Consequence — check 3a regime dependence:** if spot radius w ≪ flake thickness t, the near-field ΔT is dominated by intrinsic k_PTP spreading resistance → 3a genuinely calibrates the material and **transfers** to the maser geometry. If w ≳ t, substrate + interface conductance dominate → 3a calibrates *the rig*, not the material. **w/t is the single most load-bearing unknown for 3a.** (At the published values — w plausibly tens of µm, t = 0.5 mm — w ≪ t would hold comfortably; neither number is confirmed for the actual dataset samples.)
...
5. **Cowley-Semple ODMR dataset — RESOLVED (identity + existence), metadata pending.** What exists (from the 2026-06-26 "linewidths" thread, Glasgow/MIT/Imperial):
   ...
   **Routing update (2026-07-04): asks re-route to Angus directly.** Oxborrow makes the introduction **Monday 2026-07-06 — he asked to be reminded that morning; send the reminder.** Question list for Angus, priority order (informed by the §7.T5 rig-geometry note; the published-record partial answers are in parentheses):
   1. **Per-sample form + flake/plate thickness + lateral size — 0.1% AND 0.01% d₁₄.** Decides 3b's geometric input and the 3a w/t regime. (Published protonated record: crystals = 0.5-mm polished plates; the only published 0.1% sample is a 100 nm evaporated *film* — which are yours?)
   2. **Substrate material and mounting/adhesive.** (Published: glass slide + paraffin wax — same for these samples?)
   3. **Objective NA or measured spot size at sample.** (cw path shows only the f = 30 mm doublet AC254-030-AB; NAP sample used C240TMD-B, f = 8 mm — different spot, do not pool. w sets w/t.)
   4. **Laser power at sample and duty cycle:** wavelength (520 nm per Mena Methods — confirm); threshold current; slope efficiency (W/A) or measured power at each current. *Without this, absolute ΔT(P) is uncalibrated; the ν(I) slope and the concentration ratio still are.*
...
7. **p-terphenyl thermal properties and coefficients — largely sourced; ρ still open.** k (floor), c_p, and df_spin/dT have primaries in §6T. **Nat. Commun. pull DONE (2026-07-04):** Singh et al. (Ajoy group), Nat. Commun. 16, 10530 (2025) — RT-valid X–Z slope **−101 kHz/K** (region-III fit, Fig. 2B(iii)); headline 247 kHz/K excluded (T_xy at the phase transition). Full grading in §6T; constant single-sourced in `provenance/constants.py`. Note the hoped-for error bar did not materialise — the main text prints the slope bare. **ρ remains the one open pull.**
