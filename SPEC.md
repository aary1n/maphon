...
5. **Cowley-Semple ODMR dataset — RESOLVED (identity + existence), metadata pending.** What exists (from the 2026-06-26 "linewidths" thread, Glasgow/MIT/Imperial):
   ...
   **Routing update (2026-07-04): asks re-route to Angus directly.** Oxborrow makes the introduction **Monday 2026-07-06 — he asked to be reminded that morning; send the reminder.** Question list for Angus, priority order (informed by the §7.T5 rig-geometry note; the published-record partial answers are in parentheses):
   1. **Per-sample form + flake/plate thickness + lateral size — 0.1% AND 0.01% d₁₄.** Decides 3b's geometric input and the 3a w/t regime. (Published protonated record: crystals = 0.5-mm polished plates; the only published 0.1% sample is a 100 nm evaporated *film* — which are yours? *Mann SI checked 2026-07-04: pattern re-confirmed — 0.01% crystals = 0.5 mm plates, 0.1% only as 100 nm film, films preferred "for maximal contrast"; d₁₄ samples still absent from print. Ask stands, sharpened.*)
