...
def test_planning_pins_carry_the_ratified_planning_values():
    ...
    independent arithmetic before pinning, per house rule. RE-PINNED
    AGAIN 2026-07-22 (C0 graduation 190 -> 200, `C0_PLANNING`): every
    Δf_max-derived pin scales by exactly sqrt(199/189) = 1.0261140545 —
    verified against the prior pins by independent arithmetic before
    re-pinning. SUPERSEDED dated records: gate 1 ≈ 537.4 kHz, P5
    point/lo/hi = 1.0747e7 / 5.392e6 / 1.2953e7 (C0 = 190 era,
    2026-07-18); gate 1 ≈ 479 kHz, P5 = 9.571e6 / 5.183e6 / 11.378e6
    (ticklish-possum, 2026-07-15). Gate 2 hangs off tan_delta_MIN and
    is unchanged (≈ 24.0 kHz). Windows are quoted-precision, not new
    tolerances."""
    pins = planning_threshold_pins()
    assert 550.9e3 < pins["gate1_threshold_hz_point_branch"] < 551.9e3
    ...
    assert p5["point"] == pytest.approx(1.1028e7, rel=1e-3)
    assert p5["lo"] == pytest.approx(5.5325e6, rel=1e-3)
    assert p5["hi"] == pytest.approx(1.3291e7, rel=1e-3)
